...
def create_2_subsets(parasitized,uninfected,seed):
    ...
    train_set, val_set, train_lb, val_lb = train_test_split(cells, lb, test_size=0.2, random_state=my_seed)
    
    return train_set,val_set,train_lb,val_lb;

def data_to_torch(train_set,val_set,train_lb,val_lb):
    train_set_torch  = torch.from_numpy(train_set).float()
    val_set_torch = torch.from_numpy(val_set).float()
    train_lb_torch  = torch.from_numpy(train_lb).float()
    val_lb_torch = torch.from_numpy(val_lb).float()
    
    return train_set_torch,val_set_torch,train_lb_torch,val_lb_torch;
